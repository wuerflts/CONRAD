...
public class DMIP_ParallelBeam {
	...
	public Grid1D rampFiltering(Grid1D sinogram, RampFilterType filter){
		...
		else if(filter == RampFilterType.SHEPPLOGAN)
		{
			...
			{
				ramp.setAtIndex
			(i, -2.0f / (float)( Math.pow(Math.PI, 2) *(4 * Math.pow(i,2) -1.0f ) ) );
			}
			for(int i = paddedSize/2;i<paddedSize;i++)
			{
				final float tmp = paddedSize -i;
				...
				ramp.setAtIndex
				(i, -2.0f / (float)( Math.pow(Math.PI, 2) *(4 * Math.pow(tmp,2) -1.0f ) ) );
			}
		}
		else
		{
		...
		{
			sinogramF.multiplyAtIndex
			(p, ramp.getRealAtIndex(p), ramp.getImagAtIndex(p));
		}
